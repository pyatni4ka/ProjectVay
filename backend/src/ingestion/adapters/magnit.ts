...
const DEFAULT_MAGNIT_PRICES_PATH = process.env.MAGNIT_PRICES_PATH ?? "backend/data/magnit-prices.json";

export const magnitAdapter: IngestionAdapter = {
  id: "magnit",
  kind: "prices",
  licenseRisk: "high",
  ingest: async (context) => {
    const priceSignals = loadPriceSignals(DEFAULT_MAGNIT_PRICES_PATH, context.nowISO);
