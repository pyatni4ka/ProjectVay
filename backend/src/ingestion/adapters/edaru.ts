...
const DEFAULT_EDA_RU_SEEDS = (process.env.EDA_RU_SEED_URLS ?? "")
  .split(",")
  .map((item) => item.trim())
  .filter(Boolean);

export const edaRuAdapter: IngestionAdapter = {
  id: "eda_ru",
  kind: "recipes",
  licenseRisk: "high",
  ingest: (context) => ingestEdaRu(context)
};

export async function ingestEdaRu(context: IngestionContext): Promise<{
  products: [];
  ...
  for (const url of DEFAULT_EDA_RU_SEEDS) {
    ...
    try {
      ...
      const recipe = parseRecipeJSONLDFromHTML(html, "eda.ru", url);
      if (recipe) {
        recipes.push(recipe);
      }
    } catch {
