...
- В карточке рецепта `Готовлю` показывает подтверждение и списывает сопоставленные ингредиенты из партий.
...
- Экспорт/импорт JSON и удаление локальных данных через UI.
