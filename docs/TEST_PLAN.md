...
- Для режима `День` цель и меню адаптируются к уже съеденным КБЖУ из Apple Health (записи Yazio).
- Для режима `Неделя` используется базовая дневная цель без агрессивной коррекции.
- На экране отображаются: цель КБЖУ, съедено КБЖУ, остаток КБЖУ и таргет на следующий приём пищи.
- Блок “Рекомендации на следующий приём” пересчитывается по требуемому КБЖУ следующего приёма.
