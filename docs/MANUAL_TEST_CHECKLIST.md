...
| iPhone: one-shot скан | Достаточно один раз навести камеру, после фиксации кода можно сразу убрать телефон |  |  |
| iPhone: miss в режиме `Добавление` | Если товар не найден во внешних базах, создаётся автошаблон `Товар <штрихкод>` |  |  |
...
| Внешние barcode-провайдеры без ключей | По умолчанию используются RU fallback-каталог + бесплатные `barcode-list.ru` и семейство Open Facts (`Food/Beauty/Pet/Products`); `EAN-DB`/RF включаются отдельно |  |  |
