...
- Списание через штрихкод:
  - в сканере отдельный режим `Списание`;
  - в этом режиме товар не создаётся автоматически, скан применяется только к уже существующему инвентарю;
  - быстрый сценарий: скан -> `Быстро −1`/списание количества из партии.
- Адаптивное меню по фактическому потреблению КБЖУ:
  - чтение `dietaryEnergyConsumed`, `dietaryProtein`, `dietaryFatTotal`, `dietaryCarbohydrates` из Apple Health (данные из Yazio);
  - строгий расчёт остатка КБЖУ на день;
  - расчёт целевого КБЖУ на следующий приём пищи с учётом оставшихся приёмов;
  - рекомендации блюд для следующего приёма на основе требуемого КБЖУ.
